...
export async function processNativeConcatStream(
  ...
  try {
    ...
    for (let i = 0; i < files.length; i++) {
        fileMetas.push(await parseWebMFile(files[i]));
    }
    ...
    if (writable) {
        ...
        await writable.write(firstMeta.segmentInfo);
        totalBytesWritten += firstMeta.segmentInfo.length;
...
export async function processNativeTrimStream(
  ...
  try {
     ...
     if (writable) {
        ...
        await writable.write(meta.segmentInfo);
        totalBytesWritten += meta.segmentInfo.length;
     ...
     for (let cIdx = 0; cIdx < meta.clusters.length; cIdx++) {
         ...
         // Fast rejection based on cluster timecode
         if (cluster.timecode > endMs + 5000) break; // Past end time (+ buffer)
